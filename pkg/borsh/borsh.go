...
func ReadBytesArray(r io.Reader, n, nBytes int) ([][]byte, error) {
	result := make([][]byte, n)
	for i := range n {
		bs, err := read(r, nBytes)
		if err != nil {
			return nil, err
		}
		result[i] = bs
...
func getReadFunc(r io.Reader) ReadFunc {
	return func(n int) ([]byte, error) {
		return read(r, n)
	}
}

var deserializerType = reflect.TypeFor[BorshDeserializer]()

func deserialize(t reflect.Type, r io.Reader) (interface{}, error) {
	if t.Implements(deserializerType) {
		v := reflect.New(t)
		m, ok := v.Interface().(BorshDeserializer)
		if !ok {
			return v.Elem().Interface(), nil
		}
		result, err := m.DecodeBorsh(r, getReadFunc(r))
		if err != nil {
			return nil, err
		}
...
func deserializeUint128(r io.Reader) (interface{}, error) {
	d, err := read(r, 16)
	if err != nil {
		return nil, err
	}
