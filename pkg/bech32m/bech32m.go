...
func toBytes(chars string) ([]byte, error) {
	...
	for i := 0; i < len(chars); i++ {
		...
		if index < 0 {
			return nil, fmt.Errorf("invalid character not part of charset: %v", int(chars[i]))
		}
...
func DecodeNoLimit(bech string) (string, []byte, error) {
	...
	if !bech32VerifyChecksum(hrp, decoded) {
		...
		return "", nil, fmt.Errorf("invalid checksum (expected %v got %v)", expected, actual)
	}
...
func DecodeToBase256WithTypeNoLimit(bech string) (string, byte, []byte, error) {
	...
	if len(data) < 1 {
		return "", 0, nil, fmt.Errorf("invalid bech32 string length %d", 0)
	}
